...
const Page = () => {
  ...
  const deleteTask = async (id: string) => {
    try {
      const response = await axios.delete(
        `http://localhost:3400/api/deletetask/${id}`,
        {
          headers: {
  ...
  return (
    <div className="flex flex-row relative backdrop-blur-sm">
      ...
      <div className="w-full ">
        <Button
          className="cursor-pointer bg-black text-white clear-right p-4 py-6 mt-14 ml-14 w-[150px] "
        ...
        <div className="w-[80%]">
          {Data.map((task: Task, index: number) => (
            ...
            >
              <div
                className="bg-white p-4 border  shadow-md rounded-3xl cursor-pointer mt-5 ml-5 flex flex-row w-fit
            "
              >
                <div className="w-full">
                ...
                <div className="w-[30%] flex flex-col justify-evenly">
                  <Button
                    className="cursor-pointer bg-red-400 p-4 text-white w-[70px] h-[50px] md:h-[50px] lg:w-[120px]"
          ...
          <div></div>
          {areyouSure && (
            <div className="absolute top-1/3 left-1/2 z-30 backdrop-opacity-100">
              <div className="border h-[140px] w-[330px] flex flex-col shadow-2xl rounded-2xl  p-2 bg-white opacity-100">
                <h1 className="text-3xl text-black flex items-center justify-center font-bold font-serif mt-2">
                  Are You Sure?
                </h1>
                <div className="flex  justify-center items-center mt-6 gap-3">
                  <Button
                    className="cursor-pointer h-[40px] border border-red-100"
                    onClick={() => setareyousure(false)}
                  >
                    Cancel
                  </Button>
                  <Button
                    className="bg-red-600 text-white cursor-pointer h-[40px]"
