...
async function createProject(req, res) {
  try {
    ...
    if (!req.user) {
      return res
        .status(401)
        .json({ verified: false, message: "Unauthorized request" });
    }
    ...
    const images = await uploadToS3(req.file);
...
async function getProjects(req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Something went badly wrong", error: error.message });
  }
}
...
async function addTask(req, res) {
  ...
  try {
    ...
    res
      .status(400)
      .json({ message: "Bad request Madarchod", error: error.message });
  }
}
...
async function deleteTask(req, res) {
  if (!req.user) {
    return res
      .status(401)
      .json({ verified: false, status: 401, message: "Unauthorized" });
  }
...
async function addMember(req, res) {
  ...
  try {
    ...
    if (!user) {
      return res
        ...
        .json({
          verified: false,
          status: 404,
          message: "Member Must be Registered With Platform",
        });
    }
    ...
    if (member) {
      return res
        .status(403)
        .json({ message: "Same Member Canoot be Added Multiple Times" });
    }
...
async function MyMembers(req, res) {
  ...
  try {
    ...
    if (!members) {
      return res
        .status(404)
        .json({ message: "Not Found", verified: false, statuscode: 404 });
    }
...
async function groupProjects(req, res) {
  ...
  try {
    ...
    return res
      ...
      .json({
        message: "Found",
        projects: groupProjects.map((project) => project.Project),
      });
  } catch (error) {
    return res
      .status(500)
